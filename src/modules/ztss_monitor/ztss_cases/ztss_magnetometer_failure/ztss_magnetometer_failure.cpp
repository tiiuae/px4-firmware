#include "ztss_magnetometer_failure.hpp"

ZtssCaseMagnetometerFailure::ZtssCaseMagnetometerFailure(ModuleParams *parent)
	: ModuleParams(parent)
{
	ztss_use_case_magnetometer_failure_pub_.advertise();

	const hrt_abstime hysteresis_us = (hrt_abstime)(1_s * MAG_FAILURE_HYSTERESIS_TIME_S);

	mag_failure_hysteresis_.set_hysteresis_time_from(false, hysteresis_us);
}

void ZtssCaseMagnetometerFailure::update_subscribed_values()
{
	bool any_updated = false;

	if (sensors_status_mag_sub_.updated()) {
		sensors_status_mag_sub_.update(&sensors_status_mag_input_);
		any_updated = true;
	}
...
bool ZtssCaseMagnetometerFailure::evaluate_sensor_type(
	const bool healthy[MAG_MAX_SENSOR_INSTANCES],
	...
	uint8_t &sensor_margin)
{
	...
	for (uint8_t i = 0; i < MAG_MAX_SENSOR_INSTANCES; i++) {
	...
	case ztss_monitor_use_case_output_s::INFO: {
		...
		sensor_margin = (ratio > MAG_MARGIN_LOW_THRESHOLD)
...
void ZtssCaseMagnetometerFailure::construct_internal_business_data()
{
	// --- Evaluate magnetometer sensors ---
	uint8_t mag_severity = ztss_monitor_use_case_output_s::INFO;
	uint8_t mag_margin   = ztss_monitor_use_case_output_s::MARGIN_HIGH;
	mag_failure_ = evaluate_sensor_type(
			       sensors_status_mag_input_.healthy,
			       sensors_status_mag_input_.inconsistency,
			       sensors_status_mag_input_.device_ids,
			       sensors_status_mag_input_.device_id_primary,
			       MAG_INCONSISTENCY_WARN,
			       MAG_INCONSISTENCY_CRITICAL,
			       mag_severity,
			       mag_margin);

	severity_ = mag_severity;
	margin_   = mag_margin;

	// --- Update hysteresis ---
	const hrt_abstime time_now = hrt_absolute_time();

	mag_failure_hysteresis_.set_state_and_update(mag_failure_, time_now);
}

void ZtssCaseMagnetometerFailure::execute_use_case_safety_evaluation()
{
	if (!updated_subscribers_) {
		return;
	}

	const bool sensor_failed = mag_failure_hysteresis_.get_state();
...
void ZtssCaseMagnetometerFailure::publish_use_case_status()
{
	use_case_output_.timestamp = hrt_absolute_time();
	ztss_use_case_magnetometer_failure_pub_.publish(use_case_output_);
}
