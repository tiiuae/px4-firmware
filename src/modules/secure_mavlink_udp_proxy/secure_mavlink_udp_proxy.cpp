    ...
    if (!skip_sending) {
      ...
      // Generate a new key into packet
      {
        ...
        if (conf.proxy.encrypted_key_exchange_packet) {
          ...
          constexpr size_t packet_size{256      // RSA key size
                                       - 2 * 32 // 2* hash_function_output_size
                                       - 2};

          const size_t packets{(key_packet_current_len + packet_size - 1) /
                               packet_size};
          ...
          for (size_t i{0}; i < packets; ++i) {
            ...
            rsa_crypto.encrypt_data(
                RSA_PUB_KEY_IDX, key_packet + i * packet_size, chunk_size,
