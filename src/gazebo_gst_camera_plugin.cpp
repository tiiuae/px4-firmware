...
// gzerr <<"rate"<< this->rate<<"\n";
  ...
  if (useVaapi) {
    gst_bin_add_many(GST_BIN(pipeline), dataSrc, conv, filterNV12, encoder, filterH264, parser, payload, sink, NULL);
  } else {
    gst_bin_add_many(GST_BIN(pipeline), dataSrc, conv, encoder, filterH264, parser, payload, sink, NULL);
  }

  // Link all elements
  if (useVaapi) {
    if (gst_element_link_many(dataSrc, conv, filterNV12, encoder, filterH264, parser, payload, sink, NULL) != TRUE) {
      gzerr << "ERR: Link all the elements failed. \n";
      return;
    }
  } else {
    if (gst_element_link_many(dataSrc, conv, encoder, filterH264, parser, payload, sink, NULL) != TRUE) {
      gzerr << "ERR: Link all the elements failed. \n";
      return;
    }
  }
                              ...
                              const std::string &format)
{
  ...
  if (gst_buffer_map(frameBuffer, &mapInfo, GST_MAP_WRITE)) {
    ...
    if (this->convFbImgToI420) {
      ...
      memcpy(mapInfo.data, (uchar *)image, size);
    }
