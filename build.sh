...
hsm_build_cmd_fw="docker run --rm -e SIGNING_KEY=${SIGNING_KEY} -e PKCS11_MODULE_PATH=${PKCS11_MODULE_PATH} -e SIGNING_ARGS=${SIGNING_ARGS} -e SIGNING_TOOL=${SIGNING_TOOL} -v ${HSM_TOKENDIR}:/softhsm/tokens -v ${script_dir}:/px4-firmware/sources ${hsm_iname_env} ./packaging/build_px4fw.sh"
